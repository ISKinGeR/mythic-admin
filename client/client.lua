...
AddEventHandler('Core:Shared:Ready', function()
    exports['mythic-base']:RequestDependencies('Admin', {
        ...
        Keybinds:Add('admin_menu', 'HOME', 'keyboard', '[Admin] Open Admin Menu', function()
            id = "not important"
            Callbacks:ServerCallback('Admin:checkme', id, function(response)
                if response then 
                    Admin:OpenMenu()
                end
            end)
...
AddEventHandler('Characters:Client:Logout', function()
    ...
    TriggerServerEvent("Admin:ChangeCharacter")
end)
...
RegisterNetEvent("FixVehicleThing", function(v)
    if v then
        if Vehicles.Repair:Full(v) then
            print("Vehicle Fixed!")
        else
            print("Vehicle not fixed, ERROR!")
        end
    end
end)
...
RegisterNUICallback("GetOnlineAdminList", function(data, cb)
    id = "ALL"
    Callbacks:ServerCallback('Admin:GetAdminDATA', id, function(response)
        cb(response)
    end)
end)

RegisterNUICallback("GetAdminDATA", function(data, cb)
    if data and data.params and data.params.id then
        local id = data.params.id
        Callbacks:ServerCallback('Admin:GetAdminDATA', id, function(response)
            cb(response)
        end)
    else
        print("[Error] Invalid data received. Missing params or id.")
        cb({ error = "Invalid data structure" })
    end
end)

...
RegisterNetEvent('Admin:Client:Menu:RecievePermissionData', function(userData, permission, permissionName, permissionLevel)
    if not _hasMenu then
        if test then print(json.encode(userData), json.encode(permission), json.encode(permissionName), json.encode(permissionLevel)) end
        _hasMenu = true
        _IsAdmin = true
        SendNUIMessage({
            type = "SET_USERDATA",
...
RegisterNUICallback("OpenInvThing", function(data, cb)
    print("KR IS HERE")
    if data.targetSource then
    else
        print("No targetSource found")
    end

    local id = data.targetSource
    -- Callback to the server to perform action
    Callbacks:ServerCallback("Admin:Holdup:Do", id, function(s)
...
RegisterNUICallback('GetPlayerHistory', function(data, cb)
	cb({
        ...
        police = GlobalState["Duty:police"] or 1,
